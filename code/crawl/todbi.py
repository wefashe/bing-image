...
def get_image_listByDate(year=2024, month=1, area="cn"):
    ...
    month 月份
    """
    headers = {"User-Agent": fc.user_agent(), "Referer": "https://www.todaybing.com"}
    data = {"year": year, "month": month, "area": area, "action": "ajax_get_all"}
    ...
    for tag in tags:
        list.append(
            {
                "date": date.date_to_str(
                    date.str_to_date(tag.div["data-date"], "%Y-%m-%d")
                ),
